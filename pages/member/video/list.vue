<template>
    <div  class="content-box">
        ...
        <a-row class="content-info" v-if="list != null && list.length>0">
...
export default {
    ...
    methods: {
        async getData(){
            try {
               const res = await this.$axios.get(api.getVideoList,{params: this.queryParam})
                if (res.code != 1) {
                    this.$message.error(
                        res.message,
                        3
        ...
        create(){
            this.$router.push({ name: "member-video-create"})
        },
        view(e){
            this.$router.push({ name: "video-id",params:{id:e}})
        },
        edit(e){
            this.$router.push({ name: "member-video-edit",
                query:{
                    id:e,
                }
            })
        ...
        async postDelete(formData){
            try {
                const res = await this.$axios.post(api.postVideoRemove,formData)
                if (res.code != 1) {
                    this.$message.error(
                        res.message,
                        3
