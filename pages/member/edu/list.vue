...
export default {
    ...
    methods: {
        async getData(){
            try {
               const res = await this.$axios.get(api.getEduList,{params: this.queryParam})
                if (res.code != 1) {
                    this.$message.error(
                        res.message,
                        3
        ...
        create(){
            this.$router.push({ name: "member-edu-create"})
        },
        view(e){
            this.$router.push({ name: "course-id",params:{id:e}})
        },
        edit(e){
            this.$router.push({ name: "member-edu-edit",
                query:{
                    id:e,
                }
            })
        ...
        async postDelete(formData){
            try {
                const res = await this.$axios.post(api.postEduRemove,formData)
                if (res.code != 1) {
                    this.$message.error(
                        res.message,
                        3
